...
describe('usePagesAPI - Pruebas del Hook Real', () => {
  ...
  it('verifica la estructura real del hook', () => {
    ...
    // Debug: vamos a ver qué devuelve realmente tu hook
    console.log('Propiedades del hook:', Object.keys(result.current))
    console.log('Valores del hook:', result.current)
    
    // Verifica las propiedades básicas que debería tener
  ...
  it('verifica que error tiene la estructura correcta del estado inicial', () => {
    ...
    expect(result.current.error.message).toBe('El servicio no está disponible. Intenta nuevamente en unos minutos.')
  })
  ...
  it('prueba las funciones disponibles en el hook', () => {
    ...
    console.log('Funciones disponibles en el hook:', functions)
    
    // Si hay funciones, verifica que son funciones
  ...
  it('verifica el estado completo inicial del hook', () => {
    ...
    const expectedInitialState = {
      ...
      error: {
        status: 503,
        message: 'El servicio no está disponible. Intenta nuevamente en unos minutos.'
      }
    }
